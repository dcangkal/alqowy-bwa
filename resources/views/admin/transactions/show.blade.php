            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-10 flex flex-col gap-y-5">
                <div class="flex flex-row gap-x-10">
                    ...
                    <div class="flex flex-col gap-y-10">    
                        ...
                        @if($subcribeTransaction->is_paid)
                        ...
                        <div>
                            ...
                            <h3 class="text-indigo-950 text-xl font-bold">Rp {{$subcribeTransaction->total_amount}}</h3>
                        </div>
                        <div>
                            <p class="text-slate-500 text-sm">Checkout Date</p>
                            <h3 class="text-indigo-950 text-xl font-bold">{{$subcribeTransaction->created_at}}</h3>
                        </div>
                        <div>
                            <p class="text-slate-500 text-sm">Subscription Date</p>
                            <h3 class="text-indigo-950 text-xl font-bold">{{$subcribeTransaction->subcription_start_date}}</h3>
                        </div>
                        <div class="">
                            <p class="text-slate-500 text-sm">Student</p>
                            <h3 class="text-indigo-950 text-xl font-bold">{{$subcribeTransaction->user->name}}</h3>
                        </div>
                    </div>
                    <div>
                        <img src="{{Storage::url($subcribeTransaction->proof)}}" alt="">
                    </div>
                </div>
                <hr class="my-5">
                @if($subcribeTransaction->is_paid)
                @else
                <form action="{{route('admin.subcribe_transactions.update', $subcribeTransaction)}}" method="POST">
                    @csrf
                    @method('PUT')
